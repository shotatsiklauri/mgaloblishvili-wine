...
if (!DATASET) {
  fail('NEXT_PUBLIC_SANITY_DATASET is not set. Add it to .env.local before running this script.')
}
...
const assetRefCache = new Map<string, { _type: 'reference'; _ref: string } | undefined>()
  ...
  plans.push({
    ...
    build: async () => {
      ...
      const images = [image1, image2].filter(
        (img): img is { _type: 'image'; asset: { _type: 'reference'; _ref: string } } => !!img,
      )
      return {
        slug: slugField(id),
        sortOrder: index + 1,
        ...
        ...(region.subtitle
          ? { subtitle: bilingual(region.subtitle, kaRegion?.subtitle ?? region.subtitle) }
          : {}),
        bodyBlocks: paragraphs(region.body, kaRegion?.body ?? []),
        images: images.map((img) => ({ ...img, _type: 'regionImage', _key: randomUUID() })),
      }
    },
  })
})
...
enContent.wines.items.forEach((wine: Wine, index: number) => {
  ...
  plans.push({
    ...
    build: async () => {
      ...
      return {
        itemId: wine.id,
        category: { _type: 'reference' as const, _ref: `wineCategory-${categoryId}` },
        sortOrder: index + 1,
        name: bilingual(wine.name, kaWine?.name ?? wine.name),
        grapeOrigin: bilingual(wine.grapeOrigin, kaWine?.grapeOrigin ?? wine.grapeOrigin),
        descriptionLines: paragraphs(wine.description, kaWine?.description ?? []),
        ...(heroImage ? { heroImage } : {}),
  ...
  plans.push({
    ...
    build: async () => {
      ...
      return {
        slug: slugField(id),
        sortOrder: index + 1,
        title: bilingual(experience.title, kaExperience?.title ?? experience.title),
        sections,
        ...(heroImage ? { heroImage } : {}),
        ...(image1 ? { image1 } : {}),
        ...(image2 ? { image2 } : {}),
...
async function main() {
  console.log('')
  console.log(EXECUTE ? '=== Sanity seed (EXECUTE) ===' : '=== Sanity seed (dry run) ===')
  ...
  for (const plan of plans) {
    ...
    if (client) {
      await client.createOrReplace({ _id: plan.id, _type: plan.type, ...fields })
      console.log(`  ✓ written`)
    } else {
